...
namespace DevelopmentWithADot.LinqQueryCache
{
	public sealed class ExpressionEqualityComparer : IEqualityComparer<Expression>
	{
		...
		private void VisitLambda(LambdaExpression expression)
		{
			...
			{
				this.hashCode ^= expression.Name.GetHashCode();
			}
		...
		#region Equality
		private Boolean Visit(Expression x, Expression y)
		{
		...
		private Boolean VisitUnary(UnaryExpression x, UnaryExpression y)
		{
			return ((x.Method == y.Method) &&
				   (this.Visit(x.Operand, y.Operand)));
		}

		private Boolean VisitBinary(BinaryExpression x, BinaryExpression y)
		{
			return ((x.Method == y.Method) &&
		...
		private Boolean VisitMethodCall(MethodCallExpression x, MethodCallExpression y)
		{
			return ((x.Method == y.Method) &&
		...
		private Boolean VisitConditional(ConditionalExpression x, ConditionalExpression y)
		{
		...
		private Boolean VisitConstant(ConstantExpression x, ConstantExpression y)
		{
			return (Object.Equals(x.Value, y.Value));
		}

		private Boolean VisitInvocation(InvocationExpression x, InvocationExpression y)
		{
			return ((this.Visit(x.Expression, y.Expression)) &&
				   (this.VisitExpressionList(x.Arguments, x.Arguments)));
		}

		private Boolean VisitLambda(LambdaExpression x, LambdaExpression y)
		{
			return ((this.Visit(x.Body, y.Body)) &&
				   (this.VisitParameterList(x.Parameters, y.Parameters)));
		}

		private Boolean VisitListInit(ListInitExpression x, ListInitExpression y)
		{
			return ((this.VisitNew(x.NewExpression, y.NewExpression)) &&
				   (this.VisitElementInitializerList(x.Initializers, y.Initializers)));
		}

		private Boolean VisitMemberAccess(MemberExpression x, MemberExpression y)
		{
			return ((x.Member == y.Member) &&
				   (this.Visit(x.Expression, y.Expression)));
		}

		private Boolean VisitMemberInit(MemberInitExpression x, MemberInitExpression y)
		{
			return ((this.Visit(x.NewExpression, y.NewExpression)) &&
				   (this.VisitBindingList(x.Bindings, y.Bindings)));
		}

		private Boolean VisitNew(NewExpression x, NewExpression y)
		{
		...
		private Boolean VisitNewArray(NewArrayExpression x, NewArrayExpression y)
		{
			return (this.VisitExpressionList(x.Expressions, y.Expressions));
		}

		private Boolean VisitParameter(ParameterExpression x, ParameterExpression y)
		{
			return ((x.Type == y.Type) && (x.IsByRef == y.IsByRef));
		}

		private Boolean VisitTypeIs(TypeBinaryExpression x, TypeBinaryExpression y)
		{
			return ((x.TypeOperand == y.TypeOperand) &&
				   (this.Visit(x.Expression, y.Expression)));
		}

		private Boolean VisitBinding(MemberBinding x, MemberBinding y)
		{
		...
		private Boolean VisitMemberAssignment(MemberAssignment x, MemberAssignment y)
		{
			return (this.Visit(x.Expression, y.Expression));
		}

		private Boolean VisitMemberMemberBinding(MemberMemberBinding x, MemberMemberBinding y)
		{
			return (this.VisitBindingList(x.Bindings, y.Bindings));
		}

		private Boolean VisitMemberListBinding(MemberListBinding x, MemberListBinding y)
		{
			return (this.VisitElementInitializerList(x.Initializers, y.Initializers));
		}

		private Boolean VisitElementInitializer(ElementInit x, ElementInit y)
		{
			return ((x.AddMethod == y.AddMethod) &&
				   (this.VisitExpressionList(x.Arguments, y.Arguments)));
		}

		private Boolean VisitExpressionList(ReadOnlyCollection<Expression> x, ReadOnlyCollection<Expression> y)
		{
			if (x == y)
			{
				return (true);
		...
		private Boolean VisitParameterList(ReadOnlyCollection<ParameterExpression> x, ReadOnlyCollection<ParameterExpression> y)
		{
			if (x == y)
			{
				return (true);
		...
		private Boolean VisitBindingList(ReadOnlyCollection<MemberBinding> x, ReadOnlyCollection<MemberBinding> y)
		{
			if (x == y)
			{
				return (true);
			...
			if ((x != null) && (y != null) && (x.Count == y.Count))
			{
				for (Int32 i = 0; i < x.Count; i++)
				{
					if (this.VisitBinding(x[i], y[i]) == false)
					{
						return (false);
					}
				}
		...
		private Boolean VisitElementInitializerList(ReadOnlyCollection<ElementInit> x, ReadOnlyCollection<ElementInit> y)
		{
			if (x == y)
			{
				return (true);
			...
			if ((x != null) && (y != null) && (x.Count == y.Count))
			{
				for (Int32 i = 0; i < x.Count; i++)
				{
					if (this.VisitElementInitializer(x[i], y[i]) == false)
					{
						return (false);
					}
				}
		...
		private Boolean VisitMemberList(ReadOnlyCollection<MemberInfo> x, ReadOnlyCollection<MemberInfo> y)
		{
			if (x == y)
			{
				return (true);
